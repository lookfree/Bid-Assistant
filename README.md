...
┌──────────────────────────────────────────────────────────────────────┐
│  前端 (Next.js)                                                        │
│  apps/web   C 端应用：读标→提纲→正文→审查→述标→导出 + 会员中心          │
│  apps/admin 运营后台：独立子域 / 独立身份（RBAC+审计，建设中）          │
└───────────────┬──────────────────────────────────────────────────────┘
                │  REST + SSE
┌───────────────▼──────────────────────────────────────────────────────┐
│  App API   apps/api   (Hono 4.12 + Bun + Drizzle + PostgreSQL)         │
│  · 钱/鉴权的唯一权威：积分账本 · 收钱吧支付 · 订阅续费 · 对账退款 · 推荐 │
│  · 同步 REST + 向智能体服务 SSE 中继；只接收 usage 上报，自己扣费       │
└───────────────┬──────────────────────────────────────────────────────┘
                │  POST /agents/bidding_agent/runs  (SSE 流)
┌───────────────▼──────────────────────────────────────────────────────┐
│  Agent 服务  services/agent  (Python + FastAPI + LangGraph/deepagents) │
│  · bidding_agent = 一个 LangGraph 工作流 = 一个自包含包                 │
│  · 节点 read → outline → content → review → present → export           │
│  · deepagent 仅用于 content 节点；仅上报 token 用量，绝不碰钱           │
└───────────────┬──────────────────────────────────────────────────────┘
                │
   PostgreSQL `bidsaas`  三 schema：public（业务+积分账本，Drizzle）
                                     langgraph（PostgresSaver 检查点）
                                     agent（可观测）
   Redis（前缀 `bid:`，Cron 分布式锁 + 缓存）   ·   MinIO（bucket `bidsaas`，文件）
```
