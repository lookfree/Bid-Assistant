...
export function useCredentialsAppendix(opts: {
  ...
  const pendingIds = isSysCreds ? placeholderFileIds(active.html).filter((id) => !(id in srcMap)) : []
  ...
  function withResolvedSrc(html: string): string {
    if (!isSysCreds) return html
  ...
  /** 点「刷新附录」：调 Task 4 的免费重建端点，成功就地替换该章 HTML；409(no_credentials/
   *  content_not_done)/404 静默收起——都不是用户点一下就能解决的错误，不值得弹窗打断。 */
  ...
  return {
    ...
    pendingResolve: pendingIds.length > 0,
    withResolvedSrc,
    stale,
    refreshing,
    refreshAppendix,
