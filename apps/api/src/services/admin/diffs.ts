...
export async function resolveDiff(id: string, opts: { operator: string }) {
  ...
  await writeAudit({ operator: opts.operator, action: "refund.write", target: `diff:${id}`, before: { resolved: before.resolved }, after: { resolved: "resolved" } })
  return { ok: true }
}
...
export async function fixUnknownPaid(id: string, opts: { operator: string }) {
  ...
  await db.update(reconcileDiffs).set({ resolved: "resolved" }).where(eq(reconcileDiffs.id, id))
  await writeAudit({ operator: opts.operator, action: "refund.write", target: `diff:${id}`, before: { resolved: "open", orderStatus: order.status }, after: { resolved: "resolved", markPaid: res } })
  return res
}
