...
export const reconcileDiffs = pgTable(
  ...
  {
    id: id(),
    billDate: text("bill_date").notNull(), // 对账日 YYYY-MM-DD（UTC 窗口）
    // amount_mismatch(金额不符) | status_mismatch(状态不符) | unknown_paid(本地未知而通道已付★最高优先)
    // | provider_missing(本地已结算而通道查无) | ledger_mismatch(缓存余额≠Σ流水) | orphan_hold(超时无了结的冻结预扣)
  ...
  (t) => ({
    ...
    typeCheck: check(
      ...
      sql`${t.diffType} in ('amount_mismatch','status_mismatch','unknown_paid','provider_missing','ledger_mismatch','orphan_hold')`,
    ),
