...
export const libraryItems = pgTable(
  "library_items",
  {
    ...
    attachments: jsonb("attachments").$type<{ fileId: string; name: string; sourceFileId?: string; ocrText?: string }[]>(), // 关联 project_files 已上传文件；sourceFileId：页图附件指向来源 PDF 的 fileId；ocrText：图片附件的前置 OCR 识别文字（services/library-ocr 后台回填）
