...
// 各纠一次，而我们一次都不知道。只读,category.read（2026-08-02 接线:运营工具,默认 ops/superadmin）。
export const bidCategoriesRouter = new Hono<{ Variables: { admin: AdminUser } }>()

/** 明细：最近的改判记录（判定 → 确认），带项目名便于回溯是哪一本标。 */
bidCategoriesRouter.get("/corrections", requirePermission("category.read"), async (c) => {
  let pg
  try {
...
bidCategoriesRouter.get("/corrections/summary", requirePermission("category.read"), async (c) => {
