...
# 图收两种形状：<p><img/></p> 与**空段+裸图**——编辑器把图当块级节点,存库时
# 「<p><img/></p>」被拆成「<p></p><img>」(2026-08-14 云上终验实锤:引导行因此配不上,
# 组名丢失、身份证不锚又缀回章末);引导行与图之间的空段一并吃进块里。
CERT_BLOCK_RE = re.compile(
    r"(?:(<p[^>]*>【([^】]{1,24})】见下图：?</p>)\s*)?"
    r"(?:<p[^>]*>(?:&nbsp;|\s)*</p>\s*)*"
    r"(?:<p[^>]*>\s*<img[^>]*data-file-id[^>]*>\s*</p>|<img[^>]*data-file-id[^>]*>)", re.S)
