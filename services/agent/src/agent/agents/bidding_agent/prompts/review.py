...
SCAN_REVIEW_RULE = """
【看不见的页怎么判（本次必须遵守，优先于上面第 1、5、6、7 条一切「缺失/未响应即高风险」的写法）】
受审的投标文件里有扫描图片页，那些页的内容你完全看不到。
凡是在给你的文字里找不到、但可能印在扫描页上的材料（构成项、证照、资质、业绩证明、
盖章表格、签字页、身份证明等）：
1. 一律不得判成「缺少/未响应」的高风险，必须降为中风险（level=中风险，tone=warning）；
2. title 以「无法核验（扫描件）：」开头，例如「无法核验（扫描件）：法定代表人身份证明」；
3. advice 写明该材料可能已在扫描页中、本次无法读取，请人工核对具体是哪一项/哪一份并确认；
4. 只有当扫描页极少、明显装不下该材料时（例如全书只有一两页扫描件），才允许判「缺少」的高风险。
在正文文字里确有内容、只是内容本身不合要求的，照常按原规则判定，不受本条影响。
"""


def scan_pages_note(scanned: list[dict]) -> str:
    """受审文件的可见性说明（拼在用户消息最前面）。没有扫描图片页 → 空串，消息与此前逐字节一致。
    只讲人话，不出现任何内部字段名——模型要知道的是「有多少页看不见、里面可能是什么」。"""
    if not scanned:
        return ""
    lines = [f"- 《{f['name']}》共 {f['pages']} 页，其中 {f['image_pages']} 页为扫描图片页，"
             "其内容对你不可见——很可能包含证照、身份证明、盖章表格、签字页。"
             for f in scanned]
    return "【受审文件可见性说明】\n" + "\n".join(lines) + "\n"
