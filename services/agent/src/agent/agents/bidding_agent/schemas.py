...
class RiskFinding(BaseModel):
    ...
    # 必须用**投标文件的章节 id**。此前无描述，模型自造 s1/s2… 这类编号，前端匹配不上就静默
    # 回落第一章——用户点哪条都跳到第一章（2026-08-06 反馈）。
    # 两种场景的键形态不同：生成的标书取提纲 chapters[].id（t3/b4），上传的线下标书没有提纲，
    # 键是解析出的节号（sec-1、sec-2，见 nodes/common._aggregate）——描述必须两种都讲，
    # 只讲 t3/b4 会让独立审查场景的模型无键可取而自造（复审 2026-08-11 指出，属预先存在）。
    target_id: str = Field(
        ..., description="点击定位用的章节 id：必须原样取自本次材料里章节自带的键，不可自行编号")
