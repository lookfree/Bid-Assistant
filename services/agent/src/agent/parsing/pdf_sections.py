...
# 「零」在落款日期里有三种打法，都得收：〇(U+3007 汉字零)、○(U+25CB 白圆圈)、Ｏ(全角字母 O)。
# 实测「广州市建设工程施工公开招标 招标文件范本.pdf」的封面日期打的是 U+25CB 的「二○二二年三月」，
# 只列 U+3007 的话这一行会被当成标题，凭空切出一节——与「金额/日期不许成标题」这条正相反。
# 半角 O 不收：它是拉丁字母，收进来会让「OA」这类真标题被当成日期。
_NUMERIC_ONLY = re.compile(
    r"^[\s\d,，.。、:：;；~～\-—/()（）%¥￥$第共页元角分年月日号"
    r"零〇○Ｏ一二三四五六七八九十百千万亿]+$")
