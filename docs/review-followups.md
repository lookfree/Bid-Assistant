...
## spec307 · code-review（review 于 2026-07，钱从严——推荐奖励引擎；3 角度：逐行/规范跨文件/安全清理）

修复：① **封顶竞态深修（钱相关，3 角度一致命中最高危）**——grantReward 原来「读 rewardedSoFar（SELECT sum）→ credits.grant」两条语句分离、无行锁：同一 inviter 名下多个被邀请人并发解锁（两笔 markPaid 各触发 onInviteeFirstPaid）会各自读到发放前的累计额、双双通过 `already+amount>cap` 判定而越发；两条 referralId 的幂等键（`referral:<id>:inviter`）不同，幂等防不住跨关系越封顶=资损。深修：照 spec302/305 串行化先例，把封顶读+capped 标记+grant 收进**同一事务同一把用户余额行锁**（导出 credits.lockUserBalanceRow 复用），rewardedSoFar 改为持锁事务内读。② 清理：referral-risk 同 IP 窗口魔数 `3600_000` 提为具名 `IP_BURST_WINDOW_MS`。留档：

| # | 位置 | 问题 | 状态 | 说明 |
|---|---|---|---|---|
| R1 | 引擎注册接线 | bindByCode 无生产调用点——loginWithPhone 首次注册流未接线，引擎在 prod 里是死的（getMyCode/list 可用，但从不建 referrals、从不解锁）。**下列 R2/R3/R4 全部汇聚到此接线点，故一并延后**：引擎当前零生产暴露 | **deferred → 注册接线（Phase-0/spec308）** | 计划自述：推荐绑定发生在注册流，spec307 不产出 /bind 写接口。接线时一并落 R2-R4 |
| R2 | Sybil 服务端指纹 | assessRisk 三道防线（设备/IP/手机）全 gate 在客户端可省略的可选入参：省略即全跳过，capPerUser 成唯一兜底 | **deferred → 随 R1** | 接线时 deviceHash/ip 从请求头/TLS 服务端派生；缺指纹本身当风控信号（冻结/拒绝），不得当跳过 |
| R3 | 钩子瞬时失败奖励丢失 | onInviteeFirstPaid 挂 markPaid 提交后 best-effort，抛错仅 console.error；重试时 markPaid 已 already_final（result.paid=false）不再触发，pending 无人重扫=已挣得的奖励永久丢失（underpayment） | **deferred → 随 R1（对账重扫）** | 接线后加对账扫：status=bound & rewardState=pending & 被邀请人已有 paid 单 → 重驱 onInviteeFirstPaid（幂等）；补 markPaid→unlock 的端到端测试 |
| R4 | assessRisk 查后插竞态 | N 个同设备/IP 并发 bind 都在任一插入前读到零 → 全部通过风控 | **deferred → 随 R1** | 接线后加 referrals(device_hash) 部分唯一索引或建关系前 advisory lock |
| R5 | 接线偏离计划留档 | spec307 把 onInviteeFirstPaid 接进了 markPaid（计划原指派 spec304 接线、本 spec 只导出钩子）；且该接线无端到端测试（referral 测试直调钩子，不经 markPaid） | **noted** | 偏离已知情：钩子幂等+守卫 pending，接线安全；端到端覆盖随 R3 补 |
| L11 | 封顶边界含等号 | `already+amount>cap` 允许恰好发到 capPerUser（450+50→500 放行） | accepted | 测试证实=预期语义（够到但不越过封顶）；毛额累计口径（不含过期/扣回负流水）亦为预期，过期不放开重挣 |
| L12 | 邀请码枚举 | resolveInviter 未鉴权未限流，6 位×31 字母表≈8.87 亿键空间 | accepted | 直接影响低（只给陌生人记账，dup/self/risk 仍拦）；bind 端点接线时加限流 |
| R6 | reward_state 建模粒度 | rewardState 是 referral 级单枚举，grantReward 写 capped、unlockAndReward 任一方发出即写 unlocked：邀请人封顶+被邀请人正常发时塌缩成 unlocked，覆盖 capped 事实（simplify 深度角度发现） | **deferred（无资金影响）** | 非资损：封顶判定读账本 rewardedSoFar（不读 rewardState），capped 事实可由「无 referral:<id>:inviter 流水」复原；rewardState 仅展示态。彻底修需按 (referral,role) 粒度建模（两列或 referral_payouts 行）=加列迁移，超 spec307 手术范围 |

## spec309 · code-review（review 于 2026-07，运营后台地基；2 角度：安全+正确性 / 清理+规范）

修复：① **登录用户名枚举（时序侧信道）**——loginAdmin 在账号不存在/停用时不跑 argon2 verify（~1ms）、存在时跑（~50-100ms），可据响应时间枚举有效 admin 账号；深修：无论存在与否都对固定 DUMMY_HASH 跑一次 verify，耗时恒定。② **种子生产禁占位口令**——seedSuperadmin 缺省 admin/ChangeMe-dev-only，无 env 时静默建 well-known 全权号；生产环境用占位口令直接 throw。③ **RequireAdmin 仅 401 才登出**——原来任何 me() 失败（瞬时 5xx/网络）都清 token 跳登录、且 catch 未受 alive 守卫；改：AdminApiError 带 status，仅 401 登出、瞬时错误乐观放行、catch 加 alive。④ 抽 **sha256Hex** 共享（C 端 auth.ts + admin-auth.ts，防哈希算法漂移）。⑤ 中间件抽 **authenticateAdmin + 复用 bearer()**（requireAdmin/requirePermission/logout 三处去重）。⑥ 迁移 0020 改简洁风（内联 FK、无 statement-breakpoint，对齐手写 0018/0019）。⑦ simplify：seedAdminRoles 批量插入。留档：

| # | 位置 | 问题 | 状态 | 说明 |
|---|---|---|---|---|
| A1 | admin 首个超管 provisioning | seedSuperadmin/seedAdminRoles 只被测试调用，src/ 无启动接线——库里没有常驻超管可登；C 端 seedConfigs 同样不在启动跑（种子显式执行惯例） | **deferred → spec310** | 用户定 C：留 spec310。届时补一次性 bootstrap（`bun run admin:bootstrap` 读 env 幂等建首个超管 + seedAdminRoles），其余账号在账号管理页由超管创建 |
| L13 | 中间件链式双查 | requireAdmin + requirePermission 若链用会各解析一次 token（2 次 DB）；实际作为二选一用，不链用 | accepted | 单独使用无冗余；spec310 各路由按需选一 |
| L14 | admin token-store 稳健性 | apps/admin 的 safeStorage 少了 apps/web 的 SecurityError 探测（隐私模式/沙箱 iframe 写 localStorage 抛错） | accepted | 独立 app 无干净跨 app 复用；影响面小，量起来再抽共享 |

## spec310 · code-review（review 于 2026-07，运营后台功能页；2 角度：钱+安全+正确性 / 清理+规范）

修复：① **手动调积分幂等键含 Date.now() → 不幂等**（双击/重试各生成新键 → 重复给钱/扣钱）：改为客户端提供 idempotencyKey（前端每次调整生成 UUID），+幂等回归测试。② **退款路由审计失败返 422 → 运营重试 → 部分退款累计守卫放行二次退真钱**：退款成功后审计单独 try 只记日志，绝不把成功退款回报成失败。③ **差异工作台补 resolved==='open' 守卫**（防重复处置）。④ 分页统一：admin 列表路由改用 spec308 parsePagination（pageSize 上限 100 + page 校验，原裸 Number() → 无界扫描/NaN offset）+ pagedBody（补 hasMore）；列表服务返回 {items,total}。⑤ 抽 lib/pagination.pagedResult（7 处 Promise.all+count 样板）。⑥ 差异审计用独立 action（diff.resolve/diff.fix_unknown_paid），不复用 refund.write 作 action。⑦ 跨 spec：rbac 给 ops 补 config.write（spec310 角色模型「ops 管配置」）。⑧ **钱路径新增**：手动调积分不能走 grant（拒非正值），新增 credits.adminAdjust（签名/行锁/不扣穿/幂等）+ admin_adjust 类型（迁移 0021）。⑨ bootstrap（`bun run admin:bootstrap`，followup A1 落地）。留档：

| # | 位置 | 问题 | 状态 | 说明 |
|---|---|---|---|---|
| L15 | listUsers 搜索 | `identifier ILIKE '%q%'` 前导通配符 + user_identities 无界子查询，大表下全表扫（每次搜索） | accepted | 无钱/安全影响（参数化安全）；identity 量起来再上前缀搜/trigram 索引 |
| L16 | fixUnknownPaid 单人放行 | unknown_paid 补入账仅凭操作员断言（无通道二次核验、无双人复核），markPaid 强制 paidAmountCents=订单额 | accepted | 幂等（already_final）防重复入账 + 审计留痕；operator-asserted 是设计，量起来可加双控 |
| L17 | 中间件链式双解析 | 写路由 requireAdmin（authed 层）+ requirePermission（叶子）各解析一次 token（2 次 DB） | accepted | 语义清晰，非热路径；量起来再合并解析 |

## 推荐引擎注册接线 R1-R6 —— 已闭环（2026-07-04，merge 68ecf94）

spec307 遗留的 R1-R6 全部落地（含前端邀请链接接线）：
- **R1** loginWithPhone 首次注册带码 → bindByCode（坏码/自荐/重复/冻结不阻断注册）；前端 `/login?ref=CODE` → verifySmsCode 带 referralCode。
- **R2** deviceHash 服务端从 UA+IP 派生（sha256）；UA+IP 皆空则不派生（否则塌成常量误冻结）。
- **R3** sweepPendingReferralUnlocks（crons/referral.ts 每日 Cron）：bound+pending 重驱 unlockAndReward（延迟模式需已 paid 单；立即模式无条件兜底）；有界 LIMIT。
- **R4** bindByCode 事务化 + deviceHash advisory 锁串行化同设备并发绑定；assessRisk/freezeAndAudit 收 tx。
- **R5** markPaid→onInviteeFirstPaid 端到端测试补齐。
- **R6** rewardState 按双方合并结果置（仅双方都封顶才 capped），且更新 WHERE pending 首写胜（并发 hook+sweep 不降级 unlocked）。
...
落地方向（待排期）：
- App API：新增 `agent_model_config` 配置项（复用 `billing_configs` 表/`getConfig` 机制，或独立表）；admin-api 加 `GET/PUT /admin-api/plans/configs/agent_model`。
- agent 服务：启动时/每 run 前从 App API（或直接读 DB `public` schema 的配置）拉取 provider/model/fallback，替代 env 默认值；key 仍在 env（敏感）。
- admin 前端：套餐&配置页加"智能体模型"分区。
