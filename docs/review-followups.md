...
| # | 位置 | 问题 | 状态 | 说明 |
|---|---|---|---|---|
| C6 | 退款护栏 | provider.refund 是裸传输（无 ≤实付上限、无累计退款帽、无 paid 前置检查）；现无路由暴露不可达 | **deferred → spec306 退款流程** | spec306 接线时必须实现订单级退款服务：校验 paid 态 + refunds 表累计 ≤ amount_cents，refund_request_no 幂等 |
