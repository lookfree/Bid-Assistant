...
- 每条 item：`title`、`value`、`status`（`found` 或文件未明确则 `missing`）、`risk`（废标风险，红标）、`star`（★不可偏离），并带 **`clause_ids`**（条款 id `${secId}-cN`，对齐原型 `clauseIds`，由 spec106 `ParsedDoc.clauses` 提供）供右栏「招标文件原文」定位；`source_quote` 为可选补充摘录。
- 评分办法表：`{id, category(技术方案/商务条款/投标报价), name, score, star, desc, clause_ids, chapter_id}`（对齐原型 `ScoringRow`，`chapter_id` 是评分点→标书章节映射）。
...
- [ ] `ReadResult` schema 对齐前端 `/read`：六大分类 + item{title/value/clause_ids/source_quote/status(found|missing)/risk/star} + scoring{id/category/name/score/star/desc/clause_ids/chapter_id} + risk_summary。
