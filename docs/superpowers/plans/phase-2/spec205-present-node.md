...
- 模板样式：商务蓝 / 科技感 / 政务红 + 企业自有模板（`slideStyles` / `enterpriseTemplateStyles`）。
...
- Produces：
  ...
  - `render_pptx(deck: DeckSpec, *, template: str = "business_blue") -> bytes`：DeckSpec → .pptx 字节。
...
class DeckSpec(BaseModel):
    ...
    template: Literal["business_blue", "tech", "gov_red"] = "business_blue"
    slides: list[Slide]
...
_SAMPLE = {
    "title": "某市政务云运维 述标", "duration": 15, "template": "gov_red",
    "slides": [
...
def test_submit_deck_captures():
    ...
    assert len(get().slides) == 3 and get().template == "gov_red"
```
...
_TEMPLATE_RGB = {"business_blue": (0x1F, 0x4E, 0x79), "tech": (0x0E, 0x76, 0x90), "gov_red": (0xA8, 0x1E, 0x1E)}
...
5. 选择合适 template（business_blue/tech/gov_red）。最后调用 submit_deck 一次性提交。
"""
```
...
_DECK_ARGS = {"title": "述标", "duration": 15, "template": "gov_red", "slides": [
    {"id": "s0", "title": "封面", "kind": "cover"},
...
def test_present_node_produces_deck_and_pptx_key(monkeypatch):
    ...
    assert out["deck"]["template"] == "gov_red"
...
- [ ] `DeckSpec` 对齐原型：slides:Slide{id,title,scoring,bullets,notes,kind:cover|content|end} + qa:QA{q,a} + duration(10/15/20) + template。
