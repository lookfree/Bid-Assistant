...
最后调用 submit_chapter 提交 {chapter_id, body_html}。
"""
...
from agent.agents.bidding_agent.schemas import ChapterDraft
...
def make_content_node(ctx):
    ...
    async def content_node(state):
        ...
        # 子写手：每章一个 task；用 submit_chapter 结构化交稿
        def chapter_subagent():
            submit, get = make_submit_tool("submit_chapter", ChapterDraft, "提交本章 HTML 正文")
            return {"name": "chapter_writer", "prompt": CHAPTER_WRITER_PROMPT,
                    "tools": [submit], "_get": get}
        ...
        # 从虚拟 FS 收齐各章 HTML
        chapters = {}
        for path in await backend.list_files("chapters"):
...
def test_content_node_collects_chapters(monkeypatch):
    ...
    class _FakeBackend:
        async def list_files(self, prefix): return list(written.keys())
