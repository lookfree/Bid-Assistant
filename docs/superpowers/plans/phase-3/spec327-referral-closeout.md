...
├── services/referral-risk.ts         # 改：新增 assessAbandoned(inviteeId, abandonDays)（有效行为=任意积分消费流水）
...
- **Consumes（按既有契约，不重实现）：**
  ...
  - `services/credits` 账本（`credit_transactions` 查消费流水判定"有效行为"）。
...
| `unlockOn` | `""` \| `"invitee_first_paid"` | 空=注册绑定即发；否则被邀请人首次付费解锁 |
