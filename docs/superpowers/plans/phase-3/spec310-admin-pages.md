...
- **`billing_configs` 可视化管理**：plans 页的「配置」区读写 **同一张 `billing_configs`**（spec301 的 `getConfig/getConfigs` + 新增 `setConfig`）。改值 → 立即生效（`getConfig` 下次读到新值，无缓存层；账本 `hold` 读 `credit_cost.<op>` 即用新口径）。每次改配置写审计（前后值）。
...
- 消费既有 service，不重实现：spec302 `credits.grant/getBalance`、spec306 `createRefund`、spec301 `getConfig/getConfigs`（+ 本 spec 加 `setConfig`）、spec309 `requireAdmin/requirePermission/writeAudit/admin_users/admin_roles/admin_audit_logs`。
...
apps/api/src/
├── services/
│   ├── config.ts                  # 改：在 spec301 getConfig/getConfigs 上加 setConfig(key,value,operator)（写库 + 审计）
│   └── admin/
...
- spec309（运营后台地基）：
  - `requireAdmin: MiddlewareHandler`（校验 admin 会话，`c.set("admin", { id, username, role })`；未登录 401）。
  - `requirePermission(action: string): MiddlewareHandler`（按角色 RBAC 判定，无权 403）。
  - `writeAudit(entry: { operator: string; action: string; target: string; before: unknown; after: unknown }): Promise<void>`（落 `admin_audit_logs`）。
  - 表：`adminUsers`、`adminRoles`、`adminAuditLogs`。
  - > spec309 未最终落地前，本 spec 各 service 以这些**签名为契约**编写；route 层从 `c.get("admin")` 取 operator。若 spec309 导出名/路径不同，落地时按实际引入（中间件挂载点与 `writeAudit` 调用点已隔离，便于替换）。
...
- spec301 配置：`getConfig(key)`、`getConfigs(prefix?)`（本 spec 加 `setConfig`）。
...
| 用户 | `GET /admin-api/users`、`GET /admin-api/users/:id`、`POST /admin-api/users/:id/ban`、`POST /admin-api/users/:id/unban`、`POST /admin-api/users/:id/credits` | 读=登录；封禁=`user.ban`；调积分=`credit.grant` |
| 订单 | `GET /admin-api/orders`、`GET /admin-api/orders/:id`、`POST /admin-api/refunds` | 读=登录；退款=`refund.create` |
...
import { requireAdmin } from "../../services/admin/rbac"; // spec309 导出（路径以实际为准）
...
test("概览聚合：用户数/付费用户/今日收入/积分流水/活跃项目", async () => {
  ...
  // 今日 paid 订单 1000 分
  await db.insert(paymentOrders).values({ userId: u1, type: "recharge", amountCents: 1000, status: "paid", provider: "alipay", providerTradeNo: "T1", idempotencyKey: "o1" });
  // 积分流水若干
  ...
  expect(typeof o.activeProjects).toBe("number");
});
```
...
- `activeProjects` = `count(projects where status active)`（接 Phase 1 `projects` 表；若字段名不同按实际）。
...
export async function computeOverview(): Promise<OverviewMetrics> {
  ...
  const [[u], [p], [rev], [tx]] = await Promise.all([
  ...
  // activeProjects：接 Phase 1 projects 表（按实际 schema 引入）；无则 0
  const activeProjects = 0; // TODO: 接 projects 表 count(status active)
  return {
...
test("封禁/解封 + 审计前后值", async () => {
  ...
  const logs = await db.select().from(adminAuditLogs).where(eq(adminAuditLogs.action, "user.ban"));
...
test("手动调积分：写 credits.grant + 审计", async () => {
  ...
  const logs = await db.select().from(adminAuditLogs).where(eq(adminAuditLogs.action, "credit.grant"));
  expect(logs.length).toBeGreaterThanOrEqual(1);
});
...
- `banUser(id, {operator})` / `unbanUser(id, {operator})`：读旧 status → 更新 → `writeAudit({ operator, action:"user.ban"/"user.unban", target:`user:${id}`, before:{status}, after:{status} })`。
- `adminGrantCredits(id, { amount, reason, operator, adminId })`：调 spec302 `credits.grant(id, amount, { type:"grant", ref:`admin:${reason}`, idempotencyKey:`admin:${adminId}:${Date.now()}` })`（负向 amount 即扣减）→ `getBalance` → `writeAudit({ action:"credit.grant", target:`user:${id}`, before:{balance:旧}, after:{balance:新, amount, reason} })`。返回 `{ balance }`。
...
import { writeAudit } from "../../services/admin/rbac"; // spec309
...
export async function banUser(id: string, opts: { operator: string }) {
  ...
  await writeAudit({ operator: opts.operator, action: "user.ban", target: `user:${id}`, before: { status: u.status }, after: { status: "banned" } });
}
...
export async function unbanUser(id: string, opts: { operator: string }) {
  ...
  await writeAudit({ operator: opts.operator, action: "user.unban", target: `user:${id}`, before: { status: u.status }, after: { status: "active" } });
}
...
export async function adminGrantCredits(id: string, opts: { amount: number; reason: string; operator: string; adminId: string }) {
  ...
  await writeAudit({ operator: opts.operator, action: "credit.grant", target: `user:${id}`, before: { balance: before }, after: { balance: after, amount: opts.amount, reason: opts.reason } });
...
import { Hono } from "hono";
import { z } from "zod";
import { requirePermission } from "../../services/admin/rbac"; // spec309
...
usersRouter.post("/:id/ban", requirePermission("user.ban"), async (c) => {
  const admin = c.get("admin");
  await banUser(c.req.param("id"), { operator: admin.username });
  return c.json({ ok: true });
});
usersRouter.post("/:id/unban", requirePermission("user.ban"), async (c) => {
  const admin = c.get("admin");
...
usersRouter.post("/:id/credits", requirePermission("credit.grant"), async (c) => {
  const admin = c.get("admin");
...
test("订单列表 + 状态过滤 + 分页", async () => {
  const u = await makeTestUser();
  await db.insert(paymentOrders).values({ userId: u, type: "recharge", amountCents: 1000, status: "paid", provider: "alipay", providerTradeNo: "T1", idempotencyKey: "o1" });
  await db.insert(paymentOrders).values({ userId: u, type: "recharge", amountCents: 500, status: "created", provider: "alipay", providerTradeNo: "T2", idempotencyKey: "o2" });
...
test("订单详情含关联退款", async () => {
  const u = await makeTestUser();
  const [o] = await db.insert(paymentOrders).values({ userId: u, type: "recharge", amountCents: 1000, status: "refunded", provider: "alipay", providerTradeNo: "T3", idempotencyKey: "o3" }).returning();
...
test("finance 发起退款 → done + 审计；support → 403", async () => {
  const { app } = await import("../src/app");
  const u = await makeTestUser();
  const [o] = await db.insert(paymentOrders).values({ userId: u, type: "recharge", amountCents: 1000, status: "paid", provider: "alipay", providerTradeNo: "T4", idempotencyKey: "o4" }).returning();
  ...
  const logs = await db.select().from(adminAuditLogs).where(eq(adminAuditLogs.action, "refund.create"));
  expect(logs.length).toBeGreaterThanOrEqual(1);
...
import { Hono } from "hono";
import { z } from "zod";
import { requirePermission, writeAudit } from "../../services/admin/rbac"; // spec309
...
// 退款审批：单独挂在聚合层 /admin-api/refunds（见 index.ts），权限 refund.create
...
refundsRouter.post("/", requirePermission("refund.create"), async (c) => {
  const admin = c.get("admin");
  ...
  try {
    ...
    await writeAudit({ operator: admin.username, action: "refund.create", target: `order:${parsed.data.orderId}`, before: { status: "paid" }, after: { refundId: res.refundId, status: res.status, amountCents: parsed.data.amount } });
    return c.json(res);
  } catch (e) {
...
> 在 `routes/admin/index.ts` 加：`import { refundsRouter } from "./orders"; adminApiRouter.route("/refunds", refundsRouter);`。退款审计在 admin-api 层补写（spec306 service 内的 `auditLog` 占位由此替换/叠加为真实 `writeAudit`）。
...
**页面接线：** admin-front「plans」页分两区：①套餐区（列表 + 新建/编辑/上下架/改价）→ `GET /admin-api/plans`、`POST /admin-api/plans`、`PUT /admin-api/plans/:id`；②配置区（积分口径 `credit_cost.*` / 充值包 `recharge_packs` / 汇率 `credit_rate` / 有效期 `grant_expire_days` / 推荐规则 `referral_rules` / 代扣重试 `deduct_retry`，**读写同一张 `billing_configs`**）→ `GET /admin-api/configs`、`PUT /admin-api/configs/:key`。视觉以原型为准。

**关键：改 `billing_configs` 写库 + `getConfig` 立即读到新值（无缓存）+ 审计留前后值。**

**Files:** Modify `apps/api/src/services/config.ts`（加 `setConfig`）；Create `apps/api/src/services/admin/admin-plans.ts`、`apps/api/src/routes/admin/plans.ts`、`apps/api/test/admin-plans-configs.test.ts`

- [ ] **Step 1: 在 `services/config.ts` 加 `setConfig`（写库 + 审计）**

```typescript
import { writeAudit } from "./admin/rbac"; // spec309

// 写配置：upsert 同一张 billing_configs；改值即生效（getConfig 下次读到新值，无缓存层）。
export async function setConfig(key: string, value: unknown, opts: { operator: string }): Promise<void> {
  const before = await getConfig(key);                         // 旧值（审计前值）
  await db.insert(billingConfigs).values({ key, value })
    .onConflictDoUpdate({ target: billingConfigs.key, set: { value, updatedAt: new Date() } });
  await writeAudit({ operator: opts.operator, action: "config.write", target: `config:${key}`, before, after: value });
}
```

> `getConfig` 直接查库（spec301 实现），无内存缓存 → `setConfig` 后立即生效（账本 `hold` 读 `credit_cost.<op>` 即用新口径）。若后续引入缓存，须在 `setConfig` 失效该 key。
...
test("改 billing_config：写库 + getConfig 立即读到新值 + 审计前后值", async () => {
  await setConfig("credit_cost.read", 10, { operator: "seed" });
  expect(await getConfig("credit_cost.read")).toBe(10);
  await setConfig("credit_cost.read", 25, { operator: "ops_alice" });
...
test("充值包/推荐规则等复杂配置整体替换", async () => {
  await setConfig("recharge_packs", [{ amountCents: 100, credits: 100 }], { operator: "ops" });
  await setConfig("recharge_packs", [{ amountCents: 1000, credits: 1200 }], { operator: "ops" });
...
import { getConfigs, setConfig } from "../../services/config";
...
plansRouter.put("/configs/:key", requirePermission("config.write"), async (c) => {
  ...
  if (!parsed.success) return c.json({ error: parsed.error.flatten() }, 400);
  await setConfig(c.req.param("key"), parsed.data.value, { operator: admin.username });
  return c.json({ ok: true });
});
```
...
- `createAdmin({ username, role, password }, {operator})`：哈希密码（复用 spec309 的 hash 工具）→ 插入 `admin_users` → `writeAudit({ action:"admin.manage", target:`admin:${id}`, before:null, after:{username, role} })`。
...
import { writeAudit, hashAdminPassword } from "../../services/admin/rbac"; // spec309（hash 工具按实际导出）
...
- [ ] **订单**：列表（状态/类型过滤）/详情（含关联退款）；退款审批调 spec306 `createRefund` + 审计（`refund.create`）；finance 可、support 403。
...
- **spec309 契约**：本 spec 以 `requireAdmin`/`requirePermission(action)`/`writeAudit({operator,action,target,before,after})` + `adminUsers`/`adminRoles`/`adminAuditLogs` + `hashAdminPassword`/`makeAdminSession`(测试夹具) 为契约编写。spec309 落地后若导出名/路径不同，仅需改各文件顶部 `import`（中间件挂载点与 `writeAudit` 调用点已集中隔离）。
- **角色→权限矩阵**由 spec309 RBAC 定义；本 spec 仅声明各写接口的 `action`（`user.ban`/`credit.grant`/`refund.create`/`plan.write`/`config.write`/`admin.manage`/`audit.read`），support 在矩阵中无写权限 → 自动 403。
- **配置即生效**依赖 `getConfig` 无缓存直查库（spec301 现状）；若引入缓存层，`setConfig` 须同步失效对应 key。
- **退款审计**：spec306 service 内 `auditLog` 为占位（console），admin-api 层在 `refundsRouter` 用真实 `writeAudit` 补写 `refund.create`；二者不冲突（占位可保留或在 spec309 落地后移除）。
