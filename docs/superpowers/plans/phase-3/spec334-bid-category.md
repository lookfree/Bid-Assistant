...
- 分类**不额外计费**：判定调用只吃读标结论摘要（几 k tokens，相对读标本身可忽略），随读标步一并结算。
  **分类调用失败不得让读标步失败**——不能为一次锦上添花赔上一整轮读标费用。
...
- **模型未配置 / 调用抛错 / 超时 / 返回不合法 ⇒ 一律吞掉记日志，`value=[]`，读标步照常成功。**
...
| # | 消费点 | 代码位置 | 注入方式 | purpose |
|---|---|---|---|---|
| 1 | 提纲 | `nodes/outline.py`，紧邻 `user += package_scope(...)` | user 消息追加 | `chapters`（**只取主类别**） |
| 2 | 正文·规划轮 | `nodes/content.py:394`（`package_scope` 同处） | user 消息追加 | `planning` |
| 3 | **正文·子写手** | `nodes/content.py:373` 的 `subagents=[{... "system_prompt": CHAPTER_WRITER_PROMPT}]` | **直接拼进子写手 system_prompt** | `writing` |
| 4 | 审查 | `nodes/review.py` 的 `user` 拼接处 | user 消息追加 | `review`（**主次类别都取**）+ 行业补丁 |
| 5 | 审核表 | **`routes/generate.py` 的 `POST /generate/checklist`**——**不是图节点** | body 新增可选字段 | `checklist` |
...
- Modify: `routes/projects.ts`
  ...
  - **纠偏记录**：确认值 ≠ 判定值时写一条。**这是判定质量迭代的唯一数据来源**——不记的话，同一个判错会被一百个用户各纠一次，我们一次都不知道。
  ...
  - GET 详情回 `bidCategory`（确认值）与 `detectedCategory`（判定值 + 证据）；`?slim=1` 只回确认值
...
| 不判定 | 1 | 「零信任安全接入网关**建设工程**…采购」平手 → 交用户（**处理得对**，这是真·混合标） |
