...
**Goal:** 落地架构 §6.2：① **到期提醒 Cron**——扫描 `current_period_end` 临近（T-7/T-3/T-1 档，`billing_configs.renewal_reminder_days` 可配）且 `status=active` 的订阅，推送提醒（短信/站内），**同一订阅同一档只提醒一次**（幂等键=订阅+档）；② **手动续费**——会员中心「续费」入口 → 建 `payment_orders(type=renewal)`（金额=所选套餐当期价，**服务端从 plans 取价**）→ 复用 spec304 单笔支付链路 → 支付成功回调/轮询 → **续期 `current_period_end` + 发放当期赠送积分(grant)**，续期与发放同事务、幂等键=`renewal:<orderId>`；③ **订阅状态机 Cron**——到期未续费 `active → past_due`（宽限期 `renewal_grace_days` 可配）→ `expired`，权益降级免费版。
...
- **提醒幂等**：幂等键=`remind:<subscriptionId>:<periodEnd>:<档>`，落库去重（防 Cron 双触发骚扰用户）。
...
- **Produces**：`renewOnPaid(order)`（spec304 `markPaid` 的 renewal 分支回调）；`POST /api/membership/renew`（spec308 会员中心调）；两个已注册 Cron。
...
- [ ] 到期提醒按档触发、幂等去重、天数可配；提醒不打扰非 active 订阅。
