...
### 追加：发票交付方式（2026-07-23，用户迭代）

先按「阿里云 DirectMail 自动发邮件」实现并部署，但用户明确**改方向**：「不要做邮件发送了，直接后端上传，用户在 C 端下载」，且「有发票号不行」（要真发票文件）。故：
- **撤掉邮件整套**：删 `services/email-sender.ts`（DirectMail SingleSendMail）+ 其测试 + `ALIYUN_DM_*` env + `@alicloud/dm20151123` 依赖。（附件本也不可靠：SDK 注明附件「不支持签名鉴权调用」。）
- **改为上传+站内下载**：发票加 `file_key`（迁移 0032）。运营开具弹窗上传电子发票文件（PDF/OFD/图片）→ **经 API 中转 `putObject` 直传 MinIO**（admin 域名 `:8081` server 块无 `/s3` 代理，浏览器直传会跨源，故服务端中转）。C 端「我的发票」列表对已开票且有 file_key 的，后端现签 `/s3` 同源相对链接（`downloadUrl`）供下载。issue 不再动邮件。
- 测试：admin 14（含上传+开具回填 fileKey、非法类型 400）、C 端 7、web 49、admin 59 全绿。
