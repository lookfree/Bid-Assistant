...
> 2026-07-22 盘点：实现已合入 main（测试 24/24 绿），复选框据实补勾；遗留项见文末。
...
- [x] **Step 2: 在 `db/schema/billing.ts` 追加邀请码表、风控审计表，并给 `referrals` 补风控字段**
...
- [x] **Step 1: 写 `services/referral-errors.ts`**
...
- [x] **Step 2: 写 `services/referral.ts`（getMyCode + 内部 code→inviter 解析）**
...
- [x] **Step 3: 写 `bindByCode`（建关系；invitee 唯一 + 自荐拦截；先不发奖，奖发放放 Task 3）**
...
- [x] **Step 4: 失败测试 `test/referral-code.test.ts`**
...
- [x] **Step 1: 在 `referral.ts` 加封顶判定 + 发奖核心 + 两段发放**
...
- [x] **Step 2: 改 `bindByCode` 接入"立即发放"分支（`unlockOn` 为空时绑定即发）**
...
- [x] **Step 3: 失败测试 `test/referral-reward.test.ts`（断言"等于配置值"，不写死魔数）**
...
- [x] **Step 1: 在 `referral.ts` 实现导出钩子 `onInviteeFirstPaid`**
...
- [x] **Step 2: 边界测试补充（在 `referral-reward.test.ts`）**
...
- [x] **Step 1: 写 `services/referral-risk.ts`（判定 + 审计）**
...
- [x] **Step 2: 在 `bindByCode` 接入风控（命中 → 建 frozen 关系 + 审计，不发奖）**
...
- [x] **Step 3: 失败测试 `test/referral-risk.test.ts`**
...
- [x] **Step 1: 在 `referral.ts` 服务加 `listReferrals`**
...
- [x] **Step 2: 写 `routes/referral.ts`（Hono，从会话取当前用户）**
...
- [x] **Step 3: 失败测试 `test/referral-routes.test.ts`**
...
- [x] **邀请码**：每用户唯一一个码（`getMyCode` 幂等生成/查询）；`code→inviter` 可解析；码全局唯一。
- [x] **绑定关系**：被邀请人注册带 code → 建 `referrals`（`status=bound`/`reward_state=pending`）；`invitee_id` 唯一（重复绑 `DuplicateInviteeError`）；自荐拦截（`SelfReferralError`）。
- [x] **两段发放**：立即发放（`unlockOn` 为空，绑定即发）与延迟解锁（`unlockOn="invitee_first_paid"`，首付触发）均按配置生效。
- [x] **双方额度**：邀请人/被邀请人各得 `inviterReward`/`inviteeReward`（**测试断言等于配置值，代码不写死数值**）；奖励落 `referral_reward` 流水，带 `reward_expire_days` 有效期。
- [x] **幂等**：幂等键 `referral:<id>:inviter` / `:invitee`；重复触发 `onInviteeFirstPaid` 不重发。
- [x] **封顶**：单用户累计 `referral_reward` 达 `capPerUser` → `reward_state=capped`，不再发。
- [x] **导出钩子**：`onInviteeFirstPaid(inviteeId)` 导出且幂等、受配置条件与冻结状态守卫；供 spec304/会员激活调用。
- [x] **防刷风控**：手机/设备查重 + 同 IP 集中时段 → `status=frozen` + 写 `referral_risk_audits` 审计；冻结关系不发奖。
- [x] **路由**：`GET /api/referral/code`、`GET /api/referral/list` 接真实数据（邀请列表 + 奖励状态）。
- [x] `bun test` 全绿；迁移可重复跑；分支 `phase3/spec307-referral-engine` 合并入 `main`。

---

## 遗留（2026-07-22 盘点）

复选框对照代码/测试逐项核实，`test/referral-code.test.ts`、`referral-reward.test.ts`、`referral-risk.test.ts`、`referral-routes.test.ts`、`referral-wiring.test.ts` 共 24 例全绿（经 mbp 隧道跑 `bun test`），本文档 Task 1-6 全部步骤与验收清单均已如实勾选。以下两项在本 spec 范围内**未实现**，去向 → spec327：

- **「注册即弃」风控判定**：Architecture 一节（第 16 行）提到的异常邀请类型之一。现有 `services/referral-risk.ts` 的 `assessRisk` 只实现了 `duplicate_device` / `duplicate_phone` / `same_ip_burst` 三种判定，没有"注册后长期不激活/不付费即视为异常"的判定与冻结逻辑（`referral_risk_audits.reason` 目前也没有 `abandon` 取值落地）。→ spec327 Task C 负责。
- **`ReferralFrozenError` 错误类**：Interfaces 一节（第 64 行）的 Produces 契约里列了这个错误类，但 Task 5 实际实现（`bindByCode` 命中风控时返回 `{ rewarded: false, frozen: true }`，不抛错，注册流程按"吞错不阻断"处理）没有引入它；`services/referral-errors.ts` 目前只有 `DuplicateInviteeError`/`SelfReferralError`/`InvalidCodeError` 三个类。这是文档内部（Interfaces vs Task 5 Step 2 代码）此前就存在的不一致，非本次盘点新增缺口；是否需要补一个 `ReferralFrozenError`（例如给运营后台人工解冻场景用）留给 spec327 视需要决定。
